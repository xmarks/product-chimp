 ...
 * @return void
 */
function product_chimp_plugin_dependencies(): void {
	...
	if ( ! empty( $inactive_plugins ) ) {
		...
		/* translators: %s is a list of plugin names with links */
			esc_html__( 'The following plugins are required by this theme: %s. Please install and activate them.', 'product-chimp' ),
...
add_action( 'admin_notices', 'product_chimp_plugin_dependencies' );
 ...
 * @return void
 */
function product_chimp_plugins_suggestions(): void {
	if ( get_user_meta( get_current_user_id(), 'product_chimp_ignore_plugin_notice', true ) ) {
	...
	if ( ! empty( $inactive_plugins ) ) {
		...
		echo esc_html__( 'The following plugins are advisable to use with this theme:', 'product-chimp' );
		echo '</p><ul>';
		...
		echo '</ul>';
		echo '<p>' . esc_html__( 'Please install and activate them.', 'product-chimp' ) . '</p>';
		echo '<p><a href="#" class="st-dismiss-plugin-notice">' . esc_html__( 'Don\'t remind me', 'product-chimp' ) . '</a></p>';
		echo '</div>';
	}
}

add_action( 'admin_notices', 'product_chimp_plugins_suggestions' );

/**
 * Handle AJAX request to dismiss notice.
 */
function product_chimp_dismiss_plugin_notice(): void {
	check_ajax_referer( 'product_chimp_ignore_plugin_notice', 'nonce' );

	update_user_meta( get_current_user_id(), 'product_chimp_ignore_plugin_notice', 1 );

	wp_send_json_success();
}
add_action( 'wp_ajax_product_chimp_dismiss_plugin_notice', 'product_chimp_dismiss_plugin_notice' );
 ...
 * @return void
 */
function product_chimp_setup(): void {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on Product Chimp, use a find and replace
	 * to change 'product-chimp' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'product-chimp', get_template_directory() . '/languages' );
...
add_action( 'after_setup_theme', 'product_chimp_setup' );
 ...
 */
function product_chimp_content_width(): void {
	$GLOBALS['content_width'] = apply_filters( 'product_chimp_content_width', 640 );
}

add_action( 'after_setup_theme', 'product_chimp_content_width', 0 );
...
if ( ! function_exists( 'st_add_theme_block_categories' ) ) {
	...
	function st_add_theme_block_categories( array $categories ): array {
		$custom_category_one = array(
			'slug'  => 'product-chimp',
			'title' => __( 'Product Chimp Sections', 'product-chimp' ),
