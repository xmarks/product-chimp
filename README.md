...
Hi. I'm a starter theme called `product-chimp`. I'm a theme meant for hacking so don't use me as a Parent Theme. Instead, try turning me into the next, most awesome, WordPress theme out there. That's what I'm here for.
...
`product-chimp` requires the following dependencies:
...
1. Search for `'product-chimp'` (inside single quotations) to capture the text domain and replace with: `'megatherium-is-awesome'`.
2. Search for `product_chimp_` to capture all the functions names and replace with: `megatherium_is_awesome_`.
3. Search for `Text Domain: product-chimp` in `style.css` and replace with: `Text Domain: megatherium-is-awesome`.
4. Search for <code>&nbsp;Product_Chimp</code> (with a space before it) to capture DocBlocks and replace with: <code>&nbsp;Megatherium_is_Awesome</code>.
5. Search for `product-chimp-` to capture prefixed handles and replace with: `megatherium-is-awesome-`.
6. Search for `PRODUCT_CHIMP_` (in uppercase) to capture constants and replace with: `MEGATHERIUM_IS_AWESOME_`.

Then, update the stylesheet header in `style.css`, the links in `footer.php` with your own information and rename `product-chimp.pot` from `languages` folder to use the theme's slug. Next, update or delete this readme.

### Setup

To start using all the tools that come with `product-chimp`  you need to install the necessary Node.js and Composer dependencies :
...
`product-chimp` comes packed with CLI commands tailored for WordPress theme development :
